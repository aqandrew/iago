...
ALPHA_DEPTH = 3 ## max depth for alpha-beta
CORNER = 9.5
SIDE = 1.5
...
class IagoBot(object):
    ...
    def c_score(self):
        ...
        for i in xrange(8):
            for j in xrange(8):
                if self.board[i][j] == my_color:
                    my_score += 1
                elif self.board[i][j] == opp_color:
                    opp_score += 1
    ...
    def p_score(self):
        ...
        ## [2][2],[5][5],[2][5],[5][2]
        if self.board[2][2] == my_color:
            my_score += level_1
        elif self.board[2][2] == 1:
            opp_score += level_1
        
        if self.board[5][5] == my_color:
            my_score += level_1
        elif self.board[5][5] == 1:
            opp_score += level_1
        
        if self.board[2][5] == my_color:
            my_score += level_1
        elif self.board[2][5] == 1:
            opp_score += level_1
        
        if self.board[5][2] == my_color:
            my_score += level_1
        elif self.board[5][2] == 1:
            opp_score += level_1

        ##check for 2 value pt. spots
        ## [0][2],[0][5],[2][0],[2][7],[5][0],[5][7],[7][2],[7][5]
        if self.board[0][2] == my_color:
            my_score += level_2
        elif self.board[0][2] == opp_color:
            opp_score += level_2

        if self.board[0][5] == my_color:
            my_score += level_2
        elif self.board[0][5] == opp_color:
            opp_score += level_2

        if self.board[2][0] == my_color:
            my_score += level_2
        elif self.board[2][0] == opp_color:
            opp_score += level_2

        if self.board[2][7] == my_color:
            my_score += level_2
        elif self.board[2][7] == opp_color:
            opp_score += level_2

        if self.board[5][0] == my_color:
            my_score += level_2
        elif self.board[5][0] == opp_color:
            opp_score += level_2

        if self.board[5][7] == my_color:
            my_score += level_2
        elif self.board[5][7] == opp_color:
            opp_score += level_2

        if self.board[7][2] == my_color:
            my_score += level_2
        elif self.board[7][2] == opp_color:
            opp_score += level_2

        if self.board[7][5] == my_color:
            my_score += level_2
        elif self.board[7][5] == opp_color:
            opp_score += level_2
    ...
    def increment_direction(self,k,i,j):
        ##        0 | 1 | 2  
        ##        7 | X | 3
        ##        6 | 5 | 4
        if k == 0:
    ...
    def m_score(self):
        ...
        for i in xrange(8):
            for j in xrange(8):    
                if self.is_legal(my_color,i,j) == 1:
                    my_moves += 1
                if self.is_legal(opp_color,i,j) == 1:
                    opp_moves += 1
    ...
    def is_stable(self,stability_board,i,j,columns_full,rows_full):
        if self.board[0][0] == 0 or self.board[0][1] == 0 or self.board[1][0] == 0 or \
            ...
            ## check if on edge
                    ...
                    column = 1
            ## adjacent is stable
            elif stability_board[i+1][j] == 1 or stability_board[i-1][j] == 1:
                    column = 1
            ## adjacent is unknown - break function
            elif stability_board[i+1][j] == 8 or stability_board[i-1][j] == 8:
                return 8
            else:
                column = 0
            ...
            ## check if on edge
                ...
                row = 1
            ## adjacent is stable
            elif stability_board[i][j+1] == 1 or stability_board[i][j-1] == 1:
                row = 1
            ## adjacent is unknown - break function
            elif stability_board[i][j+1] == 8 or stability_board[i][j-1] == 8:
                return 8
            else:
                row = 0
            ...
            ## check if on edge
                ...
                positive_diagonal = 1
            ## adjacent is stable
            elif stability_board[i+1][j-1] == 1 or stability_board[i-1][j+1] == 1:
                positive_diagonal = 1
            ## adjacent is unknown - break function
            elif stability_board[i+1][j-1] == 8 or stability_board[i-1][j+1] == 8:
                return 8
    ...
    def s_score(self):
        ...
        while ( len(to_do_list) > 0 ):
            for item in to_do_list:
                ...
                j = item[1]
                ## check if stable
                if self.is_stable(stability_board,i,j,columns_full[j],rows_full[i]) == 8:
                    continue
                elif self.is_stable(stability_board,i,j,columns_full[j],rows_full[i]) == 1:
                    stability_board[i][j] = 1
                    tup = (i,j)
